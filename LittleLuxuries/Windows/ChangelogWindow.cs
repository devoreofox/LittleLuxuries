...
public class ChangelogWindow : Window, IDisposable
{
    ...
    public ChangelogWindow() : base("Little Luxuries Changelog###llux-changelog")
    {
    ...
    private static void DrawBody(string body)
    {
        foreach (var raw in body.Split('\n'))
        {
            ...
            if (string.IsNullOrWhiteSpace(line)) ImGui.Spacing();
            else if (line.StartsWith("### "))
            {
                var heading = line[4..];
                ImGui.Spacing();
                if (heading.Equals("New Tweaks", StringComparison.OrdinalIgnoreCase))
                    ImGui.TextColored(new Vector4(1f, 0.4f, 0.4f, 1f), heading + ":");
                else
                    ImGui.TextColored(HeadingColor, heading);
                ImGui.Separator();
            }
            else if (line.StartsWith("  - ")) { ImGui.Indent(); ImGui.Bullet(); ImGui.SameLine(); ImGui.TextWrapped(line[4..]); ImGui.Unindent(); }
            else if (line.StartsWith("- ")) { ImGui.Bullet(); ImGui.SameLine(); ImGui.TextWrapped(line[2..]); }
